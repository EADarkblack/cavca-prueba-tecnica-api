...
router.post(`${VERSION}/user/register`, (req, res) => {
    const {name, last_name, email, phone, password, is_admin} = req.body;
    const encriptedPass = sha1(password);
    User.create({
        name, 
        last_name, 
        ...
        is_admin})
    .then(({id, name, last_name, email, phone, address, is_admin, createdAt, updatedAt}) => {
        res.json({
            id,
            name, 
            last_name, 
...
router.put(`${VERSION}/user/:id`, validateToken, validateIdRole, (req, res) => {
    ...
    .then(() => {
        ...
        .then((data) => {
            if (data) {
                const {id, name, last_name, email, phone, is_admin, createdAt, updatedAt} = data.dataValues;
                res.json({
                    id,
                    name, 
                    last_name, 
